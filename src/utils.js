...
/**
 * Helper function that creates a `<ul><li></li></ul>` or (`<ol>`) structure out of
 * the given `modelItem` model `listItem` element. Then, it binds the created view
 * list item (<li>) with the model `listItem` element.
 ...
 * It takes a model list item element (`modelItem`) and a corresponding view list item
 * element (`injectedItem`). The view list item should be in a view list element
 * (`<ul>` or `<ol>`) and should be its only child.
...
export function injectViewList(modelItem, injectedItem, conversionApi, model) {
  ...
  // 1. Find the previous list item that has the same or smaller indent.
  // Basically we are looking for the first model item
  ...
  if (prevItem && prevItem.name === 'listItem') {
    ...
    if (nextViewList && (nextViewList.is('ul') || nextViewList.is('ol'))) {
      ...
      if (lastSubChild) {
        ...
        viewWriter.move(viewWriter.createRangeOn(lastSubChild.parent),
