 ...
 *
 * It registers the `'numberedList'`, `'letteredList'`,`'bulletedList'`, `'indentList'`
 * and `'outdentList'` commands.
 *
...
<<<<<<< Updated upstream
  ...
  init() {
    ...
    // If there are blocks allowed inside list item, algorithms using `getSelectedBlocks()`
    ...
    editing.downcastDispatcher.on('attribute:listType:listItem',
      modelViewChangeType, { priority: 'high' });
    editing.downcastDispatcher.on('attribute:listType:listItem',
      modelViewMergeAfterChangeType, { priority: 'low' });
    editing.downcastDispatcher.on('attribute:listIndent:listItem',
      modelViewChangeIndent(editor.model));
    ...
    this.listenTo(viewDocument, 'delete', (evt, data) => {
      ...
      const previousIsAListItem = positionParent.previousSibling &&
        positionParent.previousSibling.name === 'listItem';
